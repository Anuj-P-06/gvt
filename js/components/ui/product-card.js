...
export function renderProductCard(product) {
  const isFillerDisc = product.name && product.name.toLowerCase().includes('filler disc blades');
  const href = product.enquireLink || (isFillerDisc ? 'https://www.discbladesindia.com/' : 'contact.html');
  const target = (product.enquireExternal || isFillerDisc || href.startsWith('http')) ? 'target="_blank" rel="noopener noreferrer"' : '';
  return `
    <div class="group overflow-hidden rounded-2xl border border-border-light bg-white transition-shadow duration-300 hover:shadow-lg" data-category="${product.category}">
      ...
      <div class="p-6">
        ...
        <a href="${href}" ${target} class="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-ind-orange transition-colors hover:text-ind-orange-hover">
          Enquire Now
