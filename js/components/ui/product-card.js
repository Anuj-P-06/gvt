...
export function renderProductCard(product) {
  const specs = product.specs.slice(0, 2).map((spec) => `<li class="text-xs text-grey-dark">• ${spec}</li>`).join('')
  ...
  return `
    <div class="group overflow-hidden rounded-2xl border border-border-light bg-white transition-shadow duration-300 hover:shadow-lg" data-category="${product.category}">
      ...
      <div class="p-6">
        ...
        <p class="mt-2 text-sm text-grey-dark line-clamp-2">${product.description}</p>
        <ul class="mt-4 space-y-1">${specs}</ul>
